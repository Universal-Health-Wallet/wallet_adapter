...
const SignUp = ({doctorCreator, patientCreator, technicianCreator}) => {
  const [userData, setUserData] = useState({type: ''});
  const getSignUpText = () => {
    let text = ["Let's get to know", "you better"];
    switch(userData.type) {
      case "specialist": text = ["Let's make sure", "you're a doctor"]; break;
  ...
  const accountCreator = {
    "specialist": doctorCreator,
    "patient": patientCreator,
    "technician": technicianCreator,
  }
  ...
  return(
    <div className='c-signup u-m-t--45'>
      ...
      { userData.type? <SignUpForm type={userData.type} accountCreator={accountCreator[userData.type]} /> : 
